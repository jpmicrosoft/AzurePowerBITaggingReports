...
function Get-FileName ([String]$Report_Name) {    
    ...
    $file_name = $file_path + $Report_Name + $date + ".csv"
...
function Invoke-AzureSubscriptionLoop {
    ...
    foreach ($subscription_list_iterator in $subscription_list) {
        ...
        try {
            ...
            foreach ($resource_group_iterator in $resource_groups) {
                ...
                $rg_tags = (Get-AzureRmResourceGroup -Name $resource_group_iterator.ResourceGroupName)
                $Tags = $rg_tags.Tags
    ...
    $Azure_Tags | Export-Excel $Report_Name -WorksheetName $Report_Name -AutoSize -AutoFilter
}
